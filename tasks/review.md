...
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Command injection via unsanitized branch names**            | `utils/git.ts:260,267,284,290`                                                                    | **CRITICAL** | `branch` and `base` params interpolated directly into `execSync` shell commands. Branch name comes from user HTTP input (`req.body.branchConfig`). Shell metacharacters would be executed. |
...
| **No path restriction on git operations**                     | `routes/git.ts:18,40,77` -- any `folder` param passed to git commands                             | **HIGH**     |
| **Queue processor bypasses auth**                             | `queue-processor.ts:58` -- has explicit TODO acknowledging this                                   | **HIGH**     |
| **`removeWorktree` uses `JSON.stringify` for shell escaping** | `utils/git.ts:207`                                                                                | MEDIUM       | `JSON.stringify()` is not a proper shell escape -- `$`, backticks, `!` inside double quotes are still interpreted. Should use `execFile` (bypasses shell).                                 |
